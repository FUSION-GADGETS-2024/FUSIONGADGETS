  ...
  return (
    <div className="group bg-card border border-border rounded-xl p-3 transition-all duration-150 hover:border-border-medium hover:shadow-[0_4px_12px_rgba(0,0,0,0.08)]">
      ...
      <div className="space-y-1.5">
        ...
        <Link href={`/products/${id}`}>
          <h3 className="text-sm font-semibold text-foreground leading-tight hover:text-text-secondary transition-colors duration-150 cursor-pointer line-clamp-2">{name}</h3>
        </Link>
        ...
        {/* Price Section */}
        <div>
          <p className="text-base font-semibold text-foreground">₹{price.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
